...
```{r}
# source('passage_sediment_bip.R')
```

```{r}
df_gsu2 <- readxl::read_excel('hab_attributes_workbook.xlsx') %>%
  rename(fp_area_diff = `Floodplain restoration potential (area, Ha)`,
         temp_diff = `Shade restoration potential (average temperature, °C)`,
         spawn_diff = `Spawning gravel restoration potential ( area, m^2)`)
```
...
} else if (dir.exists(local_caleb)) {
...
p_wood <- ggplot() +
  theme_void() +
  geom_sf(data = shp_sub, color = 'black', lwd = 0.1) +
  geom_sf(data = shp_gsu_plt, aes(fill = spawn_diff/1e4), lwd = 0.1, color = 'black') +
    scale_fill_gradient2(low = 'white', 
