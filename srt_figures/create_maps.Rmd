...
plot_spawners_per_km_diag <- function(s) {
  ...
  if (s == 'Coho') {upper_lim <- 5000}
  if (s == 'Fall Chinook') {upper_lim <- 2000 }
  if (s == 'Spring Chinook') {upper_lim <- 200}
  if (s == 'Steelhead') {upper_lim <- 800 }
...
plot_spawners_per_km_asrp <- function(s) {
  ...
  if (s == 'Coho') {upper_lim <- 20000}
  if (s == 'Fall Chinook') {upper_lim <- 5000 }
  if (s == 'Spring Chinook') {upper_lim <- 400 }
  if (s == 'Steelhead') {upper_lim <- 1000 }
  ...
  shp_plt <- sub_plot %>% 
    ...
    mutate(climate = sub('.*\\.', '', scenario),
           scenario = str_extract(scenario,'.*\\.') %>%
             str_replace('\\.', ' '),
           diff_asrp = ifelse(diff_asrp > upper_lim,
                               upper_lim,
                               diff_asrp))
           #                     # ifelse(diff_per_km <= 0,
                               #        0,
...
plot_spawners_per_km_asrp <- function(s) {
  ...
  if (s == 'Coho') {upper_lim <-100}
  if (s == 'Fall Chinook') {upper_lim <- 20 }
  if (s == 'Spring Chinook') {upper_lim <- 3 }
  if (s == 'Steelhead') {upper_lim <- 5 }
  ...
  shp_plt <- sub_plot %>% 
    ...
    mutate(climate = sub('.*\\.', '', scenario),
           scenario = str_extract(scenario,'.*\\.') %>%
             str_replace('\\.', ' '),
           diff_per_km_asrp = ifelse(diff_per_km_asrp > upper_lim,
                               upper_lim,
                               diff_per_km_asrp))
                               # ifelse(diff_per_km <= 0,
                               #        0,
  ...
  p <- ggplot() +
    ...
    facet_grid(climate~scenario, 
               switch = 'y', ) +
    # scale_fill_viridis_c(na.value = 'gray50',
    #                      breaks = seq(0, upper_lim, length.out = 5),
    #                      labels = c(seq(0, upper_lim, length.out = 5)[-5], 
    #                                 paste0('>', upper_lim))) +
    scale_fill_gradient2(low = 'red',
                         mid = 'white',
                         high = 'blue',
