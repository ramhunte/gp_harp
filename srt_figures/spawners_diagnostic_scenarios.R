...
spawners_diagnostics <- lcm_results <- lapply(lcm_species, function(l) {
  ...
  read.csv(paste0('outputs/', l, '/lcm/', str_replace(l, '_', '.'), '_abundance_by_subbasin.csv')) %>%
    ...
    mutate(Current = spawners[scenario == 'Current'],
           restoration_potential = spawners - Current) 
}) %>%
  do.call('rbind',.) %>%
  group_by(species) %>%
  ...
  ungroup() %>%
  # group_by(scenario) %>%
  mutate(perc_of_basin = Current / spawners_basinwide,
  ...
  select(species, Subbasin, scenario, spawners, restoration_potential, rank, rank_wt) %>%
...
spawners_diagnostics <- spawners_diagnostics[c('species', 'Subbasin', 'Subbasin_num', 'EcoRegion', 'scenario','spawners', 'restoration_potential', 'rank', 'rank_wt')]
