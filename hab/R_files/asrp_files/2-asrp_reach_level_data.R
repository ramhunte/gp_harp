        ...
        case_when(
          ...
          !(primary_cr_only == 'y' & !Reach %in% primary_cr) ~
            ifelse(LW == 'y',
                   'y',
                   'n')))) %>%
  mutate(LW = ifelse(str_detect(Scenario_num, 'beaver|riparian|barrier|fp|lr'),
                     'n',
                     as.character(LW)),
         Floodplain = ifelse(str_detect(Scenario_num, 'wood|beaver|riparian|barrier|lr'),
                             'n',
                             as.character(Floodplain)),
         Riparian = ifelse(str_detect(Scenario_num, 'wood|beaver|fp|barrier|flp|lr'),
                           'n',
                           as.character(Riparian)),
         Barriers = ifelse(str_detect(Scenario_num, 'wood|beaver|fp|riparian|flp|lr'),
                           'n',
                           as.character(Barriers)),
         Beaver = ifelse(str_detect(Scenario_num, 'wood|fp|barrier|riparian|flp|lr'),
                         'n',
                         as.character(Beaver))) %>%
  left_join(., wood_data) %>%
