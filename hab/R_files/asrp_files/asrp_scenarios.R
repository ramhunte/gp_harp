...
asrp_scenarios <- read.csv('Excel_Files/ASRP_scenarios.csv')
scenario.nums <- c(levels(unique(asrp_scenarios$Scenario_num)), "Current_asrp")
scenario.years <- c(2040, 2080, 2019)

if (branch == "dev") {
  Outputs_dir = paste0("Outputs/", branch, "/hab.scenarios/", fishtype)
} else if (branch == "master") {
  Outputs_dir = paste0("Outputs/", branch, "/", version, "/hab.scenarios/", fishtype)
} else {
  if (dir.exists(paste0("Outputs/feature/", branch, "/hab.scenarios/", fishtype)) == F) {
    dir.create(path = paste0("Outputs/feature/", branch, "/hab.scenarios/", fishtype), recursive = TRUE)}
  
  Outputs_dir = paste0("Outputs/feature/", branch, "/hab.scenarios/", fishtype)
}

# 2. Begin loop of scenario years (Current, 2040 and 2080) ----
asrp.scenarios.loop1 <- lapply(scenario.years, function(x){
  
  source("R_files/asrp_files/asrp_prep.R", local = TRUE)
  ...
  asrp.scenario.loop2 <- lapply(scenario.nums, function(y){
    if (x == 2019 & y == "Current_asrp" | x == 2040 & y %in% c("scenario_1", "scenario_2", "scenario_3") | x == 2080 & y %in% 
      ...
      source("R_files/asrp_files/fl_to_gsu.R", local = TRUE)
      
      source("R_files/asrp_files/primary_cr.R", local = TRUE)
      
      source("R_files/asrp_files/movement.R", local = TRUE)
      
      source("R_files/asrp_files/gsu_list.R", local = TRUE)
      ...
      source("R_files/asrp_files/asrp_cap.R", local = TRUE)
      
      source("R_files/asrp_files/movement_2.R", local = TRUE)
      
      source("R_files/asrp_files/asrp_prod.R", local = TRUE)
      
      source("R_files/asrp_files/asrp_spawn.R", local = TRUE)
      
      source("R_files/asrp_files/asrp_prespawn.R", local = TRUE)
      
      source("R_files/asrp_files/data_organization.R", local = TRUE)
...
source("R_files/asrp_files/asrp_diagnostics.R", local = TRUE)
