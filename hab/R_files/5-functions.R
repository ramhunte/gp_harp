...
create_scenarios <- function(dataset) { # other mehtod could be map_dfr
  ...
  years <- lapply(scenario.years, function(scenario) {
    ...
    filter(!(year == 1900 & Scenario_num != 'Historical'),
           !(year == 2019 & Scenario_num %in% c('scenario_1', 'scenario_2', 'scenario_3', growth_scenarios, 'dev_and_climate', 'Historical')),
