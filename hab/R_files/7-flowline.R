flowline_noculv <- flowline %>%
  ...
  mutate(
         ...
         temp_diff_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                            ...
                            curr_temp - hist_temp),
         temp_diff_2040_prespawn = mwmt_to_prespawn_func(F2040_temp - curr_temp) + cc_mid_prespawn,
         temp_diff_2080_prespawn = mwmt_to_prespawn_func(F2080_temp - curr_temp) + cc_late_prespawn,
         temp_diff_prespawn = mwmt_to_prespawn_func(temp_diff_rear),
         curr_temp = rear_temp, 
