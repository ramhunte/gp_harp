flowline_noculv <- flowline %>%
  ...
  mutate(
         ...
         temp_diff_2040_noaction_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                                               mwmt_to_mdm_func(noaction_2040 - curr_temp),
                                               noaction_2040 - curr_temp),
         temp_diff_2080_noaction_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                                               mwmt_to_mdm_func(noaction_2080 - curr_temp),
                                               noaction_2080 - curr_temp),
         temp_diff_2040_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                                      mwmt_to_mdm_func(tm_2040 - curr_temp),
                                      tm_2040 - curr_temp),
         temp_diff_2080_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                                      mwmt_to_mdm_func(tm_2080 - curr_temp),
                                      tm_2080 - curr_temp),
         temp_diff_rear = ifelse(species %in% c('spring_chinook', 'fall_chinook'),
                            mwmt_to_mdm_func(curr_temp - hist_temp),
                            curr_temp - hist_temp),
         temp_diff_2040_prespawn = tm_2040 - curr_temp,
         temp_diff_2040_prespawn_noaction = noaction_2040 - curr_temp,
         temp_diff_2080_prespawn = tm_2080 - curr_temp,
         temp_diff_2080_prespawn_noaction = noaction_2080 - curr_temp,
         temp_diff_prespawn = curr_temp - hist_temp,
         curr_temp = rear_temp, 
         hist_temp = curr_temp - temp_diff_rear,
         tm_2040 = rear_temp_mid + temp_diff_2040_rear,
         tm_2080 = rear_temp_late + temp_diff_2080_rear,
         tm_2040_cc_only = rear_temp_mid + temp_diff_2040_noaction_rear,
         tm_2080_cc_only = rear_temp_late + temp_diff_2080_noaction_rear,
