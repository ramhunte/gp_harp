...
#Updates
In October 2020 we received updated thermalscape data from John Winkowski which included both baseline (2013-2019) and future (2040, 2080) temperatures.  Because of this, we recalculated the equation used to convert between the thermalscape and riverscape datasets.  The follwoing new files were created manually in ArcGIS:

* thermalscape_all: This file combines all of the new thermalscape datasets which included one dataset for each baseline year as well as one for each of the future years.  Additionally, in this dataset we join the old thermalscape dataset from 2019 using a spatial join.
* thermalscape_geom: This file contains the geometry of thermalscape_all as well as a unique identifier for each of the lines (OBSPRED_ID)
* riverscape_thermalscape_pts_update.txt: This file was created through a spatial join (closest) of thermalscape_geom to the riverscape data point dataset.
* thermalscape_noaaid_update: THis file was created through a spatial join in which we joined the thermalscape unique identifier to the flowline using `closest`
...
write.csv(thermalscape_temps, '../../hab/Inputs/temperature_inputs/thermalscape_temps_old.csv')
```
...
mwmt_comp_plot <- ggplot(mwmt_comp, aes(x = meanT, y = mwmt)) +
  ...
  ylim(12, 28)+
  xlim(12, 28) +
  annotate("text", x = 25, y = 15, label = 'y = 1.02x + 3.81')+
  annotate("text", x = 25, y = 14, label = expression(paste(R^2, '= 0.76'))) +
  annotate("text", x = 25, y = 13, label = "p ~ 0.0")+
  xlab('August ADA') +
  ylab('7-DADM')+
  theme_bw() +
  theme(
...
mdm_rear_comp_plot <- ggplot(mdm_rear_comp, aes(x = meanT, y = mdm_rear)) +
  ...
  ylim(12, 28)+
  xlim(12, 28) +
  annotate('text', x = 25, y = 15, label = 'y = .96x + .61') + 
  annotate('text', x = 25, y = 14, label = expression(paste(R^2, ' = 0.69'))) +
  annotate ('text', x = 25, y = 13, label = 'p ~ 0.0') +
  xlab('August ADA') +
  ylab('June 1-21 ADM')+
  theme_bw() +
...
save_plot('../../temp_paper/conversion_plots.tiff', conversion_plot, ncol = 2, base_asp = 1.1, compression = 'lzw')
```
