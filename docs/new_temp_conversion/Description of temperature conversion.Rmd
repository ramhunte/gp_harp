...
summary(lm_mwmt)
thermalscape_data %>% 
  mutate(mwmt_thermal = predict(lm_mwmt, newdata = .))
```
...
summary(lm_mdm_rear)
thermalscape_data %>% mutate(mdm_rear_thermal = predict(lm_mdm_rear, newdata = .))
```
...
```{r}
write.csv(thermalscape_data, '../../hab/Inputs/temperature_inputs/thermalscape_temps.csv')
```


The psu temperature data are currently summarized in termps of mean of daily max July-Aug.  Use the following equation, found by performing linear regression with riverscape mean of daily max and 
