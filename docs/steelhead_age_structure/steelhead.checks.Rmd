...
# Run the model for steelhead to generate a model.all object
...
# Respawners
We are targeting 12% respawners, and assume that all of those are females. 

```{r}
model.all[1,100, , , "Current"] %>%
  rowSums() %>%
  as.data.frame %>%
  rownames_to_column('lifestage') %>%
  rename(Freq = 2) %>%
  filter(str_detect(lifestage, 'firstspawn|kelt')) %>%
  separate(lifestage, into = c('lifestage', 'spawn_status')) %>%
  group_by(spawn_status) %>%
