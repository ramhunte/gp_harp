...
#ri_to_surv <- nls(log(surv) ~ a*ri^b, data = dat_sk, start = list(a = 1, b = 0))
...
dat_sk %>% 
  mutate(surv_rescale = range.rescale(surv) %>% round(2), 
         ri = ri %>% round(1)) %>%
  rename(Year = migration_year, Q = RankedQ, RI = ri, p = surv,  p_rescale = surv_rescale) %>%
  select(Year, Q, RI, p, p_rescale) %>%
  flextable::flextable()
```
...
zimmerman <- read_excel('../../../flow_paper/Zimmerman 2015.xlsx',
           skip = 2, 
           n_max = 17) 

zimmerman %>%
 select(`year (i)`, Surv) %>%
  rename(migration_year = `year (i)`) %>%
  left_join(dat_sk_peakfq) %>%
