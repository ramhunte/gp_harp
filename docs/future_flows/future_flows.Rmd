...
```{r}
df_coeffs <- mods %>%
  mutate(coeffs = map(fit, ~round(coefficients(.x), 3)),
         r2 = map(fit, ~round(summary(.x)$r.squared, 2))) %>%
  unnest_wider(coeffs) %>%
  unnest(r2) %>%
  mutate(climate_model = ifelse(climate_model == 4.5, 'RCP 4.5', 'RCP 8.5'),
         era = ifelse(era == 2050, 'Mid-century', 'Late-century'),
...
p5 <- ggplot(cig_plot) +
  ...
  labs(x = 'Recurrence interval (years)',
       y = 'Change from current',
       color = NULL) +
  theme(legend.position = c(.78, .92))

ggsave('Fig5_peak_flow_change.tiff', p5, height = 4, width = 6.5, dpi = 300, compression = 'lzw')
...
p3 <- ggplot() +
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy = 1L),
                     limits = c(0, .25)) +
  geom_text(aes(50, .22),
            label = paste0("log[e]*p==",
                           round(coefficients(ri_to_surv)[1],2),
                           "*'RI'^",
                           round(coefficients(ri_to_surv)[2], 3)), 
            parse = TRUE, 
            size = 4) +
  labs(x = 'Recurrence interval (years)',
...
#RI_geomean <- ri.to.surv(2)



ri_to_surv_scalar <- function(x, 
                              mean_ri = 2) {
  
  surv_geomean <- ri.to.surv(mean_ri)
  
  surv_rescaled <- (ri.to.surv(x) - surv_geomean) / surv_geomean
  
  return(1 + surv_rescaled)
  
}


ri_to_surv_scalar(1:100)
...
#                mutate(surv_rescale = surv.scalar(surv)),
#              aes(ri, surv_rescale)) +
#   geom_line(data = sk_pred %>%
#               mutate(surv_pred_rescale = surv.scalar(surv_pred)),
#                      aes(ri, surv_pred_rescale)) +
#   scale_x_log10(limits=c(1, 100)) +
#   geom_vline(xintercept = 1.5, lty = 2)


plot(1:100, ri_to_surv_scalar(1:100), type = 'l')
...
```{r}
table_1 <- dat_z %>% 
  mutate(surv_scalar = ri_to_surv_scalar(ri) %>% round(2), 
         ri = ri %>% round(1)) %>%
  rename(Year = year, RI = ri, p = surv,  p_scalar = surv_scalar) %>%
  select(Year, RI, p, p_scalar) %>%
  mutate(p = round(p, 3))
...
p4 <- daily_ch %>%
  ...
  #geom_hline(yintercept = 874, lty = 2)


ggsave('Fig4_incubation_timing.tiff', p4, height = 4, width = 6, dpi = 300, compression = 'lzw')
...
# Figure of log pearson III
```{r}
# Hints taken from here: http://eaheisman.blogspot.com/2013/06/flood-frequency-curves-in-ggplot2.html

ch_obs <- read.delim2('PEAKFQ_CHEHALIS_AT_PORTER_1970_1999_DAILY.EMP',
            skip = 7) %>%
  mutate_all(as.numeric) %>%
  rename(EXC_Prob = B17BEst) %>%
  mutate(EXC_trans = qnorm(EXC_Prob),
         RI = 1/EXC_Prob)

ch_logp <- read.delim2('PEAKFQ_CHEHALIS_AT_PORTER_1970_1999_DAILY.EXP', 
                       sep = "\t",
                       skip = 20,
                       strip.white = TRUE,
                       header = FALSE) %>%
...
xbreaks = c(.01, .02, .05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.98, 0.99)
xaxis <- qnorm(xbreaks)
  
p2 <- ggplot() +
  cowplot::theme_half_open() +
  geom_point(data = ch_obs, aes(EXC_trans, RankedQ * 0.0283168)) +
  geom_line(data = ch_logp, aes(EXC_trans, Conf_Low * 0.0283168), lty = 2) +
  geom_line(data = ch_logp, aes(EXC_trans, Conf_Up * 0.0283168), lty = 2) +
  geom_line(data = ch_logp, aes(EXC_trans, Estimate * 0.0283168)) +
  scale_y_log10(expand = c(0, 0),
                limits = c(100,5000))  +
  annotation_logticks(sides = "l") +
  scale_x_reverse(breaks = xaxis,
                  labels = xbreaks * 100,
                  limits = qnorm(c(.995, 0.01)),
                  expand = c(0, 0)) +
  labs(x = 'Annual exceedence probability (%)',
       y = bquote('Discharge ' ~(m^3*s^-1)))

ggsave('Fig2_logpIII.tiff', p2, height = 3, width = 5, dpi = 300, compression = 'lzw')
