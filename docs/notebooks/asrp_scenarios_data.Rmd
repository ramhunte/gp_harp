...
coho_ss <- total_coho$length[total_coho$Habitat == 'SmStream']
coho_lr <- total_coho$length[total_coho$Habitat == 'LgRiver']
coho_tidal <- total_coho$length[total_coho$Habitat == 'Tidal']
coho_total <- coho_ss + coho_lr + coho_tidal
...
curr_fp_summer <- total_coho_fp$Area[total_coho_fp$hab.scenario == 'Current' & total_coho_fp$life.stage == 'summer']
curr_fp_winter <- total_coho_fp$Area[total_coho_fp$hab.scenario == 'Current' & total_coho_fp$life.stage == 'winter']
hist_fp_summer <- total_coho_fp$Area[total_coho_fp$hab.scenario == 'Historical' & total_coho_fp$life.stage == 'summer']
hist_fp_winter <- total_coho_fp$Area[total_coho_fp$hab.scenario == 'Historical' & total_coho_fp$life.stage == 'winter']
...
coho_ss_1 <- scenario_1_length$length[scenario_1_length$Habitat == 'SmStream']
coho_lr_1 <- scenario_1_length$length[scenario_1_length$Habitat == 'LgRiver']
coho_tidal_1 <- scenario_1_length$length[scenario_1_length$Habitat == 'Tidal']
coho_total_1 <- coho_ss_1 + coho_lr_1 + coho_tidal_1
...
coho_ss_2 <- scenario_2_length$length[scenario_2_length$Habitat == 'SmStream']
coho_lr_2 <- scenario_2_length$length[scenario_2_length$Habitat == 'LgRiver']
coho_tidal_2 <- scenario_2_length$length[scenario_2_length$Habitat == 'Tidal']
coho_total_2 <- coho_ss_2 + coho_lr_2 + coho_tidal_2
...
coho_ss_3 <- scenario_3_length$length[scenario_3_length$Habitat == 'SmStream']
coho_lr_3 <- scenario_3_length$length[scenario_3_length$Habitat == 'LgRiver']
coho_tidal_3 <- scenario_3_length$length[scenario_3_length$Habitat == 'Tidal']
coho_total_3 <- coho_ss_3 + coho_lr_3 + coho_tidal_3
...
There are `r coho_total` km of river habitat available to coho for rearing in the Chehalis River basin.  This includes `r coho_ss` km of small stream habitat, `r coho_lr` km of large river habitat, and `r coho_tidal` km of tidal habitat.  Additionally, there are `r curr_fp_summer` ha of summer floodplain habitat and `r curr_fp_winter` ha of winter floodplain habitat currently available to coho, while there were `r hist_fp_summer` ha of summer floodplain habitat and `r hist_fp_winter` ha of winter floodplain habitat available to coho historically. 

#### Stream length: 

Habitat | Length (km) | Scenario 1 length (km)| Scenario 2 length (km) | Scenario 3 length (km)
--------|-------------|-----------------------|------------------------|-----------------------
Total length | `r coho_total` | `r coho_total_1` | `r coho_total_2` | `r coho_total_3`
Small stream |  `r coho_ss` | `r coho_ss_1` | `r coho_ss_2` | `r coho_ss_3`
Large river | `r coho_lr` | `r coho_lr_1` | `r coho_lr_2` | `r coho_lr_3` 
Tidal |  `r coho_tidal` | `r coho_tidal_1` | `r coho_tidal_2` | `r coho_tidal_3`
...
fp_rest_1_f <- fp2 %>%
  ...
  left_join(., asrp_reach_data %>%
              filter(Scenario_num == 'scenario_1', 
                     year == 2019) %>%
              select(noaaid, GSU, Scenario_num, Floodplain, perc_forest, LW)) %>%
  filter(
    # Floodplain == 'y',
         hab.scenario %in% c('Current', 'Historical')) %>%
    mutate(perc_forest = ifelse(perc_forest > .5, 'y', 'n')) %>%
  group_by(hab.scenario, LW, perc_forest, Floodplain) %>%
  summarize(Area = sum(Area, na.rm = T))
