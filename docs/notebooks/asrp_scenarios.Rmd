...
Where `Barriers == 'y'` for a particular GSU, we model the removal of all barriers (except natural barriers) within that GSU, regardless of restoration percentage.  No barriers are removed where `Barriers == 'n'`.  The removal of barriers results in a 0.57% increase in coho spawners for scenario 1, a 0.82% increase in coho spawners for scenario 2, and a 0.66% increase in coho spawners for scenario 3.  

### Large wood

Where `LW == 'y'` for a particular GSU, we model the effect of the addition of wood to that GSU based on the restoration percentage.  The addition of large wood affects our model in the following ways:

1. Rearing capacity:  We apply a large wood multiplier when calculating rearing capacity in large river, backwater, small stream, and floodplain side channel habitat.  We use the following equation to calculate the wood multipler for each reach: $$woodmult = woodmult_{curr} + (woodmult_{hist} - woodmult_{curr}) * rest.percent$$
where $woodmult_{curr}$ == 1, $rest.percent$ can be found in the asrp_scenarios spreadsheet.
...
$$pool.perc = pool.perc_{low.wood} + (pool.perc_{high.wood} - pool.perc_{low.wood}) * rest.percent$$ $$riffle.perc = 1 - pool.perc$$
4. Spawning: 
  + For small stream habitat we have separate `forested` and `non-forested` redd densities in reaches with slope > 1%. Under high wood scenarios we use the `forested` redd densities whereas under the low wood scenarios we only use the `forested` redd densities where the landcover is forest, and we use the `non-forested` redd densities in all other reaches. We use the following equation to calculate egg capacity under the asrp scenarios: $$egg.capacity = reach.length_{meters} * passage.percent * ((redd.density_{forested} - redd.density_{non-forested}) * rest.percent) / 1000 * fecundity$$ 
  + For large river habitat we apply a wood scalar when calculating spawning capacity in high wood scenarios.  Large river spawning capcaity in the asrp scenarios is calculated using the following equation: $$area_{spawning} * passage.percent / redd.area * fecundity * (1 + (1 - spawning.mult_{high.wood}) * rest.perc)$$

The addition of large wood results in a 1.69% increase in coho spawners for scenario 1, a 2.05% increase in coho spawners for scenario 2, and a 2.39% increase in coho spawners for scenario 3.

### Floodplain

Where `Floodplain == 'y'` for a particular GSU, we model the effect of floodplain restoration in that GSU based on the restoration percentage.  The restoration of floodplain habitat affects our model in the following ways: 
...
Restoration of floodplain habitat results in a 4.17% increase in coho spawners for scenario 1, a  4.36% increase in coho spawners for scenario 2, and a 5.92% increase in coho spawners for scenario 3. 

### Beaver

Where `Beaver == 'y'` for a particular GSU, we model the effect of beaver pond additions in that GSU based on the restoration percentage as well as the beaver intensity scalar (0.1 where managed forests account for > 50% of a GSU, 1 elsewhere).  The restoration of beaver ponds affects our model in the following ways:
...
Restoration of beaver pond habitat results in a 0.18% increase in spawners for scenario 1, a 0.28% increase in spawners for scenario 2 and a 0.32% increase in spawners for scenario 3.

### Riparian

Where `Riparian == 'y'` for a particular subbasin, we model tree planting and growth for the entire GSU.  If `Riparian == 'n'` we model tree growth in all reaches where there are existing trees (canopy opening angle is less than 170°), and apply a 75% effectiveness scalar.  Where no trees currently exist we do not model any planting or growth of trees.  Riparian restoration results in a 4.53% increase in spawners by mid century, and a 5.88% increase in spawners by late century for all three scenarios.
