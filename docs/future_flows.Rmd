...
q_sk <- c(88200,142000,40100,27600,32100,55700,132000,47600,35600,51900,76800,19300,73700,53000,110000,66700,57400)
surv <- c(9,1.2,13.7,14.4,16.7,10.2,3.8,15.6,16.4,16.5,12.7,13.5,12.9,10.8,5.8,7.3,11.4)

dat_sk <- data.frame(q_sk = q_sk,
                     surv = surv/100) %>%
  mutate(ri = q_to_ri(q_sk, peak_mv$q_cfs, 0.12))
...
data.frame(ri = 1:100) %>%
  mutate(j_pred = -1.88084588 +  -0.05511075 * ri,
         j_surv = j_pred %>% inv.logit(),
         c_surv = predict(ri_to_surv, newdata = .) %>% inv.logit()) %>%
  select(-j_pred) %>%
  gather(model,surv, j_surv:c_surv) %>%
  ggplot +
