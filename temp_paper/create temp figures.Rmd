...
temp_chg_plt_late <- ggplot() +
  theme_void() +
  geom_sf(data = st_union(sub), size = .2, color = 'gray20', fill = '#E1E1E1') +
  geom_sf(data = fig6_data %>% filter(Scenario %in% c('Late century riparian restoration', 'Late century floodplain restoration', 'Late century combined')),
          aes(color = temp_diff, size = width)) +
...
ggsave('Figure_diff.tiff', dpi = 300, height = 10, width = 8, compression = 'lzw')
```
