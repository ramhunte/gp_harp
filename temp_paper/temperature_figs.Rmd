...
ggsave('figures/fig5_no_action_temps.tiff', dpi = 300, height = 10, width = 8, compression = 'lzw')
  
```
```{r}
cc_mid = 1.024
cc_late = 2.049
fig_supplement_data <- fl_raw %>% 
  select(noaaid) %>%
  left_join(., fl_temps %>%
...
fig_supplement_data$Scenario = factor(fig_supplement_data$Scenario, levels = c('Mid-century riparian restoration', 'Late century riparian restoration', 'Mid-century floodplain restoration', 'Late century floodplain restoration', 'Mid-century combined', 'Late century combined'))

fig_supplement_data$temp_diff = factor(fig_supplement_data$temp_diff, levels = c( '0.5° — 0°', '1.5° — 0.5°', '2.5° — 1.5°', '> 2.5°'))
fig_supplement_data$width = factor(fig_supplement_data$width, levels = c('low', 'high'))


temp_chg_plt_rip <- ggplot() +
  theme_void() +
  geom_sf(data = st_union(sub), size = .2, color = 'gray20', fill = '#E1E1E1') +
  geom_sf(data = fig_supplement_data %>% filter(Scenario %in% c('Mid-century riparian restoration', 'Late century riparian restoration')),
  ...
  theme(strip.text.x = element_text(margin = margin(0, 0, .1, 0, 'cm'), size = 12),
        ...
        legend.position = 'bottom')
print(temp_chg_plt_rip)
  
ggsave('figures/fig_s2_riparian.tiff', dpi = 300, height = 4, width = 7, compression = 'lzw')
...
temp_chg_plt_fp <- ggplot() +
  theme_void() +
  geom_sf(data = st_union(sub), size = .2, color = 'gray20', fill = '#E1E1E1') +
  geom_sf(data = fig_supplement_data %>% filter(Scenario %in% c('Mid-century floodplain restoration')) %>% mutate(Scenario = 'Floodplain restoration'),
          aes(color = temp_diff, size = width)) +
  facet_wrap(~Scenario, ncol = 2) + 
  scale_color_manual(breaks = c( '0.5° — 0°', '1.5° — 0.5°', '2.5° — 1.5°', '> 2.5°'),
                      values = c( '#808080','#BEFFE8', '#00C5FF', '#004DA8'),
  ...
  theme(strip.text.x = element_text(margin = margin(0, 0, .1, 0, 'cm'), size = 12),
         strip.text = element_text(size = 12),
        legend.position = 'bottom')
print(temp_chg_plt_fp)
  
ggsave('figures/fig_s3_fp.tiff', dpi = 300, height = 4, width = 3.5, compression = 'lzw')
```
```{r}
...
temp_chg_plt_comb <- ggplot() +
  theme_void() +
  geom_sf(data = st_union(sub), size = .2, color = 'gray20', fill = '#E1E1E1') +
  geom_sf(data = fig_supplement_data %>% filter(Scenario %in% c('Mid-century combined', 'Late century combined')),
          aes(color = temp_diff, size = width)) +
  facet_wrap(~Scenario, ncol = 2) + 
  scale_color_manual(breaks = c( '0.5° — 0°', '1.5° — 0.5°', '2.5° — 1.5°', '> 2.5°'),
                      values = c( '#808080','#BEFFE8', '#00C5FF', '#004DA8'),
  ...
  theme(strip.text.x = element_text(margin = margin(0, 0, .1, 0, 'cm'), size = 12),
         strip.text = element_text(size = 12),
        legend.position = 'bottom')
print(temp_chg_plt_comb)
  
ggsave('figures/fig_s3_combined.tiff', dpi = 300, height = 4, width = 7, compression = 'lzw')
```
...
temp_plot_rest <- ggplot() +
  ...
  theme(strip.text.x = element_text(margin = margin(0, 0, .1, 0, 'cm'), size = 12),
        legend.position = 'bottom')
print(temp_plot_rest)
  
ggsave('figures/fig7_rest_temp_reduction.tiff', dpi = 300, height = 10, width = 8, compression = 'lzw')
```
```{r}
temp_chg_plt_late <- ggplot() +
  theme_void() +
  geom_sf(data = st_union(sub), size = .2, color = 'gray20', fill = '#E1E1E1') +
  geom_sf(data = fig_supplement_data %>% filter(Scenario %in% c('Late century riparian restoration', 'Late century floodplain restoration', 'Late century combined')),
          aes(color = temp_diff, size = width)) +
  ...
  theme(strip.text.x = element_text(margin = margin(0, 0, .1, 0, 'cm'), size = 12),
        legend.position = 'bottom')
print(temp_chg_plt_late)

ggsave('figures/fig6_temp_reduction.tiff', dpi = 300, height = 10, width = 8, compression = 'lzw')
```
